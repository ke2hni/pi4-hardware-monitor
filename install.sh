...
TXT
    ...
    repo_layout)
      cat <<'TXT'
Simple reason:
The installer was not run from a complete Pi 4 Hardware Monitor project folder.
...
TXT
    ...
    unsupported_hardware)
      cat <<'TXT'
Simple reason:
This installer only supports Raspberry Pi 4 hardware.

What this usually means:
The system is unsupported for this plugin installer. This is not a plugin code failure.

What to do next:
Run this installer only on a Raspberry Pi 4.
TXT
      ;;
    apt_problem)
      cat <<'TXT'
...
print_summary() {
  local exit_code="${1:-0}"
  printf '%s\n' '========== Pi 4 Hardware Monitor installer summary =========='
...
check_pi4_hardware() {
  ...
  [[ "$model" == *"Raspberry Pi 4"* ]] || fail_with_help "unsupported_hardware" "This installer only supports Raspberry Pi 4 hardware. Detected: ${model:-unknown}"
  add_summary_unique SUMMARY_ALREADY_OK "Hardware verified: ${model:-Raspberry Pi 4}"
}
